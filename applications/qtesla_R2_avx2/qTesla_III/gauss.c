...
static void gaussian_sampler(int32_t *sample, uint32_t slen)
{
	...
	while (j < slen)
	{
		...
		sample[j++] = (int32_t)(z[i - 1] * (1 ^ ((-k) & 0xfffffffffffffffe))); /* sample=z*(-1)^k */
	}
}

void sample_gauss_poly(poly z, const unsigned char *seed, int nonce)
{ // Gaussian sampler
	fastrandombytes_setseed(seed, nonce<<8);
	gaussian_sampler(z, PARAM_N);
}
